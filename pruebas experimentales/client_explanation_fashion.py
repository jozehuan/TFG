...
hfl_cs_cnn_sys = HFL_System(name='hfl_cs_fashion_cnn', **config)

# Show the data distribution on the nodes (node 0 = server) 
barplot(hfl_cs_cnn_sys.class_counter(), 'hfl_cs_fashion_cnn', ncols=4)

# Set CNN model defined in models module
hfl_cs_cnn_sys.set_model(arch = 'cs', build = nets.build_server_fashion_CNN_model)

# Tomar datos (NO HACER PORQUE SERÁN LOS DATOS DEL SERVIDOR)
data = hfl_cs_cnn_sys._flex_pool.clients.select(criteria=lambda actor_id, _: actor_id in [1])._data.data[1]
data = data[0:300]

# Train federated model
start_time = time.time()
hfl_cs_cnn_sys.train_n_rounds(n_rounds = 50, clients_per_round = 19, no_client_ids=[1])
end_time = time.time()
...
start_time = time.time()
hfl_cs_cnn_exps = hfl_cs_cnn_sys.get_explanations(data, client_id=1)
end_time = time.time()
time_exp_cnn = end_time - start_time
print(f"\n\nExplanation time (CNN): {time_exp_cnn:.2f} seconds")

# Plot segments:
plot_segments(*hfl_cs_cnn_sys.segments(data, client_id=1))

# Plot explanations
plot_heatmaps(*hfl_cs_cnn_exps)
plot_heatmaps(*hfl_cs_cnn_sys.label_explanations(data, client_id=1))
